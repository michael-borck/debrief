...
// 2. Builds the screenshot carousel by probing screenshots/debrief-1.png,
...
const REPO = 'michael-borck/debrief'
 ...
 * electron-builder.json: Debrief-{version}-arm64.dmg,
 * Debrief.Setup.{version}.exe, Debrief-{version}.AppImage,
 * debrief_{version}_amd64.deb. Browsers can't reliably distinguish Apple
 * Silicon from Intel, so macOS defaults to the arm64 DMG (the common case on
 * current hardware); an Intel DMG is offered as the alternative if one is
 * published. Linux defaults to the AppImage with the .deb as the alternative.
 */
function assetsFor(platform) {
  ...
  if (platform === 'windows') {
    return { primary: a.find((x) => x.name.endsWith('.exe')), alt: null, altLabel: '' }
  }
  return {
    primary: a.find((x) => x.name.endsWith('.AppImage')),
    alt: a.find((x) => x.name.endsWith('.deb')),
    altLabel: 'Debian/Ubuntu? Download .deb →',
  }
}
...
const SHOT_PREFIX = 'screenshots/debrief-'
...
function buildCarousel(urls) {
  ...
  urls.forEach((url, i) => {
    ...
    img.src = url
    img.alt = `Debrief screenshot ${i + 1}`
