...
- [x] **3.2b Route diarisation + transcription through the sidecar.** `public/electron/sidecar-client.js` wraps multipart POST to `/analyse` against the live sidecar port. `local-transcription-transcribe` and `local-transcription-rediarise` IPC handlers rewritten to use it. Renderer return shapes unchanged. **Known regressions:** per-chunk progress collapsed to a single 'transcribing' stage emit (sidecar /analyse is one-shot); cancel closes the HTTP connection but the sidecar keeps churning; the Sprint 2.C cluster-threshold slider is a silent no-op (pyannote 3.1's high-level API doesn't expose it — repurpose to `num_speakers` hint planned); rediarise rerun re-transcribes (single endpoint).
