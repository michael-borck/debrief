...
Three ways to get audio and video into Debrief: drag-and-drop, the file browser, or the menu bar (File → New Upload).
...
Behind the scenes, Debrief uses Electron's `webUtils.getPathForFile()` to resolve the absolute path of each dropped file. This is the modern replacement for the deprecated `File.path` property and works on Electron 32+.
...
Debrief uses the bundled `ffmpeg-static` to decode audio, which means **almost any audio or video format works**:

- **Audio**: MP3, WAV, M4A, AAC, OGG, FLAC, WMA, OPUS, AMR
- **Video**: MP4, MOV, AVI, MKV, WebM, WMV, FLV, 3GP

Whatever ffmpeg can read, Debrief can transcribe. Video files have their audio track extracted automatically.
...
The Selected Files list supports multiple files. Drop a batch, click Upload & Process, and Debrief processes them one at a time in the queue. The success toast fires per file.
...
- **First-time downloads.** The first time you transcribe with a new model, Debrief downloads it (~75-470 MB depending on choice). The first time you run diarisation, it downloads pyannote (~6 MB) and wespeaker (~25 MB). After that, all processing is fully local.
- **Duplicate detection.** If you upload a file with the same name as an existing transcript, Debrief asks before adding it.
